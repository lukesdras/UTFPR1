			...
			printf("ALUNO NAO EXISTE \n");
				...
				if(result){
					...
					const char* capacidade2 = "101";
						...
						if(row!=0){
								sprintf(linhas, "SELECT * FROM PESSOAS WHERE ID_BLOCO = 1");
								sprintf(capacidade, "SELECT CAPACIDADE FROM BLOCOS WHERE ID = 1");
								if (&conexao != NULL){
								...
								if (linhas2>=capacidadeint-1){
									...
									sprintf(temp2, "INSERT INTO LOG(ID, DATA, HORA, ACAO, ID_BLOCO, ID_PESSOA ) values ('0', '%s', '%s', 'IN', 1, '%s')",  Data, currentTime(), tag);
									sprintf(temp3, "UPDATE PESSOAS SET ID_BLOCO = 1 WHERE TAG = '%s'", tag);//
