#include <stdio.h>
#include <string.h>
#include </usr/include/mariadb/mysql.h>
#include "testar.cpp"
#include "initDB.cpp"
#include "gettime.cpp"
int main(int argc, char *argv[]){

		
	    char X[500]; //auxiliar
		memset (X,0,500); //setar tudo 0
		char temp[255];
		memset (temp,0,255); //setar tudo 0
	    char temp2[255];
		memset (temp2,0,255);//setar tudo 0
	    char temp3[255];
		memset (temp3,0,255);//setar tudo 0
		const char *tag;
		//memset (tag,0,255);
	    //printf("droga");
	    while(1){
		int i = 0;
		char *var;
		
		delay(1000);
		dadosDB();
		memset (X,0,500); //setar tudo 0
		memset (temp,0,255); //setar tudo 0
		memset (temp2,0,255);//setar tudo 0
		memset (temp3,0,255);//setar tudo 0
		//memset (tag,0,255);
		tag=readtag(X);
		std::cout << "currentDate()=" << currentDate() << std::endl;
		std::cout << "currentTime()=" << currentTime() << std::endl;
		char Data[500];
		memset (Data,0,500); //setar tudo 0
		sprintf(Data, "%s", currentDate());
		//getchar();
			sprintf(temp, "SELECT * FROM PESSOAS WHERE TAG LIKE '%%%s%%'", tag); //Dois % pois C interpreta % como comando
			if(mysql_query(&conexao, temp)){
			printf("ALUNO NAO EXISTE \n");
			}
			else{
				result = mysql_store_result(&conexao);
				if(result){
					unsigned int fields = mysql_num_fields(result);
					    row = mysql_fetch_row(result);
						if(row!=0){
							 
								sprintf(temp2, "INSERT INTO LOG(ID, DATA, HORA, ACAO, ID_BLOCO, ID_PESSOA ) values ('0', '%s', '%s', 'IN', 5, '%s')",  Data, currentTime(), tag);
								sprintf(temp3, "UPDATE PESSOAS SET ID_BLOCO = 5 WHERE TAG = '%s'", tag);//
								if (&conexao != NULL){
									mysql_query(&conexao, temp3);
									printf("FOI");
								}
								if (&conexao != NULL){
									mysql_query(&conexao, temp2);
								}
								printf("Liberado ok\n %s\n \n ", temp2);
								printf("\n%s \n \n ", temp3);
								
							 //inserir no LOG que aluno entrou
						}
						else printf("Aluno nao cadastrado\n");	
						row=0;
					while(row = mysql_fetch_row(result)){
						for(i=0 ; i<fields; i++){

						var=row[i];

						printf("   %s", var);
			
						}
				printf("\n");
				}
			}
			
		

			
		else printf("DEU ERRO");
			
		
	}
	//	else printf("banco com problema");
	}
}
