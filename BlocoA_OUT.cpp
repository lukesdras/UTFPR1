				...
				if(result){
					unsigned int fields = mysql_num_fields(result);
						...
						if(row!=0){
								...
								sprintf(temp2, "INSERT INTO LOG(ID, DATA, HORA, ACAO, ID_BLOCO, ID_PESSOA ) values ('0', '%s', '%s', 'OUT', 1, '%s')",  Data, currentTime(), tag);
								sprintf(temp3, "UPDATE PESSOAS SET ID_BLOCO = 10 WHERE TAG = '%s'", tag);//
								if (&conexao != NULL){
						...
						else printf("Aluno nao cadastrado\n");	
						row=0;
